...
local fun ino (Empty, Llist, height) = (NONE, height)::Llist
	| ino (Node (N, LST, RST), Llist, height) =
		let val Mlist = ino (RST, Llist, (height+1))
			val Nlist = ino (LST, (SOME N, height)::Mlist, (height+1))
		in  Nlist
		end
...
local
	...
	fun recover_it [] = []
		...
		if h1=h3 andalso h2=h1-1 then [Node((v2,h2), Node((v1,h1),l1,r1), Node((v3,h3),l3,r3))]
